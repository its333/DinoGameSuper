...
(function() {
    ...
    function Runner(outerContainerId, opt_config) {
        ...
        this.name = opt_config.name || 'Player';
        this.onCrash = typeof opt_config.onCrash === 'function' ? opt_config.onCrash : null;
        this.onScore = typeof opt_config.onScore === 'function' ? opt_config.onScore : null;
        this.onReady = typeof opt_config.onReady === 'function' ? opt_config.onReady : null;
        this.controlMap = opt_config.controls || Runner.keycodes;
        ...
        if (!this.outerContainerEl) {
            throw new Error('Runner container not found: ' + outerContainerId);
        }
        this.canvas = null;
        this.canvasCtx = null;
        this.tRex = null;
    ...
    Runner.prototype = {
        ...
        playIntro: function() {
            if (!this.started && !this.crashed) {
                ...
                styleSheet.innerText = '@keyframes intro { ' + 'from { width:' + Trex.config.WIDTH + 'px }' + 'to { width: ' + this.dimensions.WIDTH + 'px }' + '}';
                document.head.appendChild(styleSheet);
                //document.styleSheets[0].insertRule('@keyframes intro { ' + 'from { width:' + Trex.config.WIDTH + 'px }' + 'to { width: ' + this.dimensions.WIDTH + 'px }' + '}', 0);
        ...
        startGame: function() {
            ...
            if (!this.boundVisibilityHandler) {
                ...
                this.boundWindowBlurHandler = this.onVisibilityChange.bind(this);
                this.boundWindowFocusHandler = this.onVisibilityChange.bind(this)
            }
            document.addEventListener(Runner.events.VISIBILITY, this.boundVisibilityHandler);
        ...
        update: function() {
            ...
            if (this.activated) {
                ...
                var roundedDistance = Math.ceil(this.distanceRan);
                var playAcheivementSound = this.distanceMeter.update(deltaTime, roundedDistance);
                if (playAcheivementSound) {
                    this.playSound(this.soundFx.SCORE)
                }
                if (this.onScore && roundedDistance != this.lastReportedScore) {
                    this.lastReportedScore = roundedDistance;
                    var displayDistance = this.distanceMeter.getActualDistance(roundedDistance);
                    this.onScore(displayDistance, this);
                }
            }
            if (!this.crashed) {
        ...
        onKeyDown: function(e) {
            ...
            var jumpKey = this.controlMap.JUMP[keyCode];
            if (!this.crashed && (jumpKey || e.type == Runner.events.TOUCHSTART || e.type == Runner.events.GAMEPADCONNECTED)) {
                if (!this.activated) {
                    this.loadSounds();
                    this.activated = true
                }
            ...
            if (this.activated && !this.crashed && this.controlMap.DUCK[keyCode]) {
                e.preventDefault();
                if (this.tRex.jumping) {
        ...
        onKeyUp: function(e) {
            ...
            if (this.isRunning() && isjumpKey) {
                this.tRex.endJump()
            } else if (this.controlMap.DUCK[keyCode]) {
                this.tRex.speedDrop = false;
                this.tRex.setDuck(false)
            } else if (this.crashed) {
                var deltaTime = getTimeStamp() - this.time;
                if (restartKeys[keyCode] || this.isLeftClickOnCanvas(e) || (deltaTime >= this.config.GAMEOVER_CLEAR_TIME && this.controlMap.JUMP[keyCode])) {
                    this.restart()
                }
        ...
        destroy: function() {
            ...
            this.stopListening();
            if (this.boundVisibilityHandler) {
                document.removeEventListener(Runner.events.VISIBILITY, this.boundVisibilityHandler);
            }
            if (this.boundWindowBlurHandler) {
                window.removeEventListener(Runner.events.BLUR, this.boundWindowBlurHandler);
            }
            if (this.boundWindowFocusHandler) {
                window.removeEventListener(Runner.events.FOCUS, this.boundWindowFocusHandler);
            }
            if (this.touchController && this.touchController.parentNode) {
                this.touchController.parentNode.removeChild(this.touchController);
            }
